...
<xsl:template match="sparql:results"  xmlns="http://www.w3.org/1999/xhtml">
  <h1>Orders List</h1>
  <p><xsl:value-of select="count(sparql:result)"/><xsl:text> orders</xsl:text></p>
  <ul>
    <xsl:for-each select="sparql:result">
      
      <li><a><xsl:attribute name="href">order.php?uri=<xsl:value-of select="sparql:binding[@name='uri']"/></xsl:attribute><xsl:value-of select="sparql:binding[@name='common']"/></a><em><xsl:text> (</xsl:text><xsl:value-of select="sparql:binding[@name='order']"/><xsl:text>) </xsl:text></em></li>
    </xsl:for-each>
  </ul>
</xsl:template>
