...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
       ...
       <title>FishDelish Families</title>
    </head>
    <body>
<div id="content">
...
<div id="footer">
  <a href="http://fishdelish.cs.man.ac.uk">FishDelish</a> Project, data (c) <a href="http://www.fishbase.org">Fishbase</a>, January 2011.
</div>
    </body>
</html>
</xsl:template>
...
<xsl:template match="sparql:results"  xmlns="http://www.w3.org/1999/xhtml">
  <h1>Families List</h1>
  <p><xsl:value-of select="count(sparql:result)"/><xsl:text> families</xsl:text></p>
  <ul>
    <xsl:for-each select="sparql:result">
      
      <li><a><xsl:attribute name="href">family.php?uri=<xsl:value-of select="sparql:binding[@name='uri']"/></xsl:attribute><xsl:value-of select="sparql:binding[@name='common']"/></a><em><xsl:text> (</xsl:text><xsl:value-of select="sparql:binding[@name='family']"/><xsl:text>) </xsl:text></em></li>
    </xsl:for-each>
  </ul>
</xsl:template>
